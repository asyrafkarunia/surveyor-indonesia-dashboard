...
const ActivityTimeline: React.FC = () => {
  const getIcon = (type: string) => {
    switch(type) {
      ...
      case 'create': return { icon: 'add', bg: 'bg-slate-100', text: 'text-slate-600' };
      default: return { icon: 'info', bg: 'bg-slate-100', text: 'text-slate-600' };
    }
  };
          ...
          <div key={activity.id} className="relative flex gap-4 pl-6">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2">
                <h4 className="text-sm font-semibold text-slate-900">{activity.user}</h4>
                {activity.tag && (
