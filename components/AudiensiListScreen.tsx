    ...
    <main className="flex-1 flex flex-col h-full overflow-hidden bg-slate-50 relative">
      ...
      <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-8 shrink-0">
        <div className="flex items-center gap-2 text-sm text-slate-500">
          ...
          <span className="text-slate-900 font-medium">Daftar Surat Audiensi</span>
        </div>
      </header>
      ...
      <div className="flex-1 overflow-y-auto p-6 md:p-8 custom-scrollbar">
        <div className="max-w-7xl mx-auto flex flex-col gap-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold tracking-tight text-slate-900">Daftar Surat Audiensi</h1>
              <p className="text-sm text-slate-500 mt-1">Manajemen dan pemantauan seluruh permohonan surat audiensi yang telah dibuat.</p>
            </div>
            <button 
              onClick={onCreateNew}
          ...
          <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col md:flex-row gap-4 items-center">
            <div className="relative flex-1 w-full group">
              ...
              <input 
                className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border-slate-200 rounded-lg text-sm focus:ring-1 focus:ring-primary focus:border-primary transition-all" 
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50/50 border-b border-slate-200">
                    <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Nomor Surat Audiensi</th>
                    <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Tanggal Surat</th>
                    <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Nama Perusahaan/Instansi</th>
                    <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Nama Pimpinan/Tujuan</th>
                      ...
                      <tr key={letter.id} className="hover:bg-red-50/5 transition-colors group">
                        <td className="px-6 py-4">
                          <div className="flex flex-col">
                            <span className="text-sm font-semibold text-slate-900 font-mono tracking-tight">{letter.letter_number}</span>
                            <span className="text-[10px] text-primary font-bold uppercase mt-0.5">{letter.sector}</span>
                          </div>
                        </td>
                        <td className="px-6 py-4 text-sm text-slate-600">{formatDate(letter.date)}</td>
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <div className={`size-8 rounded flex items-center justify-center bg-slate-100 text-slate-500`}>
                              <span className="material-symbols-outlined text-lg">domain</span>
                            </div>
                            <span className="text-sm font-medium text-slate-900">{letter.company_name}</span>
                          </div>
                        </td>
                        <td className="px-6 py-4 text-sm text-slate-600">{letter.purpose}</td>
                        <td className="px-6 py-4">
            ...
            <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-between">
              <p className="text-xs text-slate-500 font-medium">Hal {page} dari {lastPage} • Total {total} surat</p>
              <div className="flex items-center gap-2">
                <button
                  className="p-1 rounded border border-slate-200 text-slate-400 hover:bg-slate-50 disabled:opacity-50"
                  disabled={page <= 1}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center gap-4">
              ...
              <div>
                <p className="text-[10px] text-slate-500 uppercase font-bold tracking-wider">Total Dikirim</p>
                <p className="text-xl font-bold text-slate-900">{stats.total_sent ?? 0}</p>
              </div>
            </div>
            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center gap-4">
              ...
              <div>
                <p className="text-[10px] text-slate-500 uppercase font-bold tracking-wider">Audiensi Mendatang</p>
                <p className="text-xl font-bold text-slate-900">{stats.upcoming ?? 0}</p>
              </div>
            </div>
            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center gap-4">
              <div className="flex-1 flex items-center gap-4">
                ...
                <div>
                  <p className="text-[10px] text-slate-500 uppercase font-bold tracking-wider">Diterima</p>
              ...
              <div className="flex-1 flex items-center gap-4 pl-4">
                ...
                <div>
                  <p className="text-[10px] text-slate-500 uppercase font-bold tracking-wider">Ditolak</p>
